...
(async function () {
  ...
  program
    .requiredOption('-R, --root <path>', 'Root path (content is relative to root')
  ...
  for (let mdxPath of mdxPaths) {
    ...
    const { frontmatter, code } = await bundleMDX({
      ...
      xdmOptions(options) {
        options.remarkPlugins = [
          ...(options.remarkPlugins ?? []),
          // remarkMdxCodeMeta,
          [remarkPrism, { plugins: ['diff-highlight', 'line-numbers'] }],
        ];
    ...
    const response = await fetch(`${process.env.API_URL}/set/${slug}`, {
      method: 'post',
      body: JSON.stringify({
        slug,
        hash,
...
function parseSeries(path) {
  ...
  const slug = path.substring(path.indexOf('content/') + 8, path.indexOf('_series') - 1);
